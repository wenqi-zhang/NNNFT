...
mfname='modelnfttrain.pth'

device = "cuda" if torch.cuda.is_available() else "cpu"
print("Using {} device".format(device))

dataset = NFTDataset('DataSet128p3.mat', 0, 0, normalization='Q', transform=torch.tensor)
...
try:
    ...
    model.load_state_dict(savedata['model'], strict=True)
except:
...
## Sample spectra with different num of channels and different power levels
    ...
    for ii in range(3):
        ...
        for jj in (0, 9):
            ...
            x, x_norm = prepareU(Uii[jj][:,idx], normalization=False)
            y, y_norm = prepareQ(Qii[jj][:,idx], normalization=True)
            x, y = x.to(device), y.to(device)
            y_hat = model(x)
            Xii.append((x[0] * x_norm).cpu().numpy())
            Yii.append((y[0] * y_norm).cpu().numpy())
            Yii_hat.append((y_hat[0] * y_norm).cpu().numpy())
    ...
    loss, losses = valid(model, dataset, loss_fn, batch_size=200)
...
torch.save(savedict, 'postprocess.pth')
